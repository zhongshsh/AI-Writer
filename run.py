...
top_p = 0.75 # 这个的范围是 0 到 1。越大，变化越多。越小，生成效果越规矩。该值控制sample的概率范围，大于top_p的预测概率才进行sample
top_p_newline = 0.9
...
if DATA_NAME == 'wangwen':
    MODEL_NAME = '../RWKV-LM/model/%s/wangwen' % DATA_NAME # 模型名
    WORD_NAME = '../RWKV-LM/model/%s/wangwen' % DATA_NAME # 这个也修改
    ...
    MODEL_NAME = '../RWKV-LM/model/%s/trained-1' % DATA_NAME # 模型名
    WORD_NAME = '../RWKV-LM/model/%s/vocab' % DATA_NAME # 这个也修改
...
for run in range(NUM_OF_RUNS):       # 写的次数
    ...
    for i in range(LENGTH_OF_EACH):  # 写的字数
        ...
        with flow.no_grad():
            if RUN_DEVICE == 'dml':
                ...
                xxx = flow.tensor(x[-ctx_len:], dtype=flow.long)[None,...]
